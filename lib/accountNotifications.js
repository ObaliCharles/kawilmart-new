...
export const buildAccountUpdateNotification = ({
    ...
    if (intersects(changedKeys, [...LEGAL_AND_ACCOUNT_FIELDS])) {
        if (changedKeys.has("verificationNotes") || changedKeys.has("legalNotes")) {
            lines.push("Your compliance notes were updated by Wilwa.");
        }
    }

    if (intersects(changedKeys, [...PROFILE_FIELDS])) {
        lines.push("Your profile details were updated by Wilwa.");
    }
    ...
    return {
        title,
        message: previewMessage || "Your Wilwa account was updated by an admin.",
        emailTitle: `Wilwa ${title.toLowerCase()}`,
        emailMessage: fullMessage || "Your Wilwa account was updated by an admin.",
