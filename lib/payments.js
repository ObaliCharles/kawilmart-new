...
const flutterwaveProvider = {
    ...
    verifyWebhook: (headers, rawBody = "") => {
        ...
        // Which header arrives depends on the account's webhook version, so
        // both schemes are accepted and each is checked the way ITS OWN version
        // defines. Treating a v3 verif-hash as an HMAC (or vice versa) rejects
        // a perfectly genuine callback.
        //   v4: flutterwave-signature = base64(HMAC-SHA256(rawBody, secret))
        //   v3: verif-hash            = the secret hash itself
        const v4Signature = headers.get("flutterwave-signature") || "";
        if (v4Signature) {
            const expected = createHmac("sha256", secret).update(rawBody, "utf8").digest("base64");
            if (matches(expected, v4Signature)) {
                return true;
            }
            // Some integrations emit the digest as hex rather than base64.
            const expectedHex = createHmac("sha256", secret).update(rawBody, "utf8").digest("hex");
            return matches(expectedHex, v4Signature);
        }

        const v3Hash = headers.get("verif-hash") || "";
        return Boolean(v3Hash) && matches(secret, v3Hash);
    },
