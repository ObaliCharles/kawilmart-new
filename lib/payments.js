...
// Flutterwave v3 has no separate sandbox host: test mode is decided purely by
// which secret key you use (FLWSECK_TEST-… vs FLWSECK-…). FLW_ENV therefore
// exists to state the intent, and is enforced below so a live key can never be
// used while testing (or a test key silently left in production).
...
const isTestSecretKey = (key = "") => /^FLWSECK_TEST-/i.test(String(key).trim());

// Guards the sandbox/live boundary. Mismatches return false, which makes the
// gateway count as unconfigured, so checkout falls back to collect-on-delivery
// instead of transacting against the wrong environment.
const flutterwaveKeyMatchesEnv = () => {
    const key = process.env.FLW_SECRET_KEY || "";
    const env = flutterwaveEnv();
    const isTest = isTestSecretKey(key);

    if (env === "sandbox" && !isTest) {
        console.error(
            "Flutterwave: FLW_ENV=sandbox but FLW_SECRET_KEY is a LIVE key. Refusing to use it — set a FLWSECK_TEST- key, or FLW_ENV=live to go live."
        );
        return false;
    }
    ...
    if (env === "live" && isTest) {
        console.error(
            "Flutterwave: FLW_ENV=live but FLW_SECRET_KEY is a TEST key. Refusing to use it — no real payments would be collected."
        );
        return false;
    }
...
// Shared by verify() and verifyByReference(), which return the same envelope
// from different endpoints. "successful" is the only status that releases goods;
// anything still in flight stays pending so a live payment is never voided (and
// its stock released) while the shopper is mid-flow.
const toFlutterwaveResult = (payload, fallbackTransactionId = "") => {
    ...
    return {
        ...
        transactionId: data.id ? String(data.id) : String(fallbackTransactionId || ""),
...
const flutterwaveProvider = {
    ...
    initiate: async ({ reference, amount, customer, redirectUrl }) => {
        const payload = await flutterwaveRequest("/payments", {
            ...
            body: {
                ...
                customizations: {
                    title: "Wilwa",
                    description: "Payment for your Wilwa order",
                },
            },
        });
    ...
    verify: async (transactionId) => {
        ...
        return toFlutterwaveResult(payload, transactionId);
    },

    // Flutterwave, unlike Pesapal, returns no transaction id when the checkout
    // is created — the id only exists once the shopper pays. So when the
    // webhook is late and the return URL carries no transaction_id, our own
    // tx_ref is the only handle we have. Without this the order would sit
    // Pending with its stock held until the webhook eventually landed.
    verifyByReference: async (reference) => {
        const payload = await flutterwaveRequest(
            `/transactions/verify_by_reference?tx_ref=${encodeURIComponent(reference)}`
        );
        return toFlutterwaveResult(payload, "");
    },
