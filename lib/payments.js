...
const flutterwaveProvider = {
    ...
    initiate: async ({ reference, amount, customer, redirectUrl, method }) => {
        ...
        const customerPayload = await flutterwaveRequest("/customers", {
            method: "POST",
            body: {
                email: customer?.email || `shopper+${reference}@wilwa.app`,
                name: { first, last },
                ...(phone ? { phone: { country_code: "256", number: phone } } : {}),
            },
        });
    ...
    verifyWebhook: (headers) => {
        ...
        const received = headers.get("verif-hash") || headers.get("flutterwave-signature") || "";
