...
import { ORDER_STATUSES, PAYMENT_STATUSES, PAYMENT_METHOD_LABELS } from "@/lib/orderLifecycle";
import { createPaymentTrackingEvent, createSellerOrderPlacedNotification, createStatusTrackingEvent } from "@/lib/orderTracking";
...
export const confirmPaidOrders = async (orders, transactionId) => {
    ...
    const customerEntry = {
        ...
        emailTitle: "Your Wilwa payment was received",
...
export const failOrders = async (orders, transactionId) => {
    ...
    await Promise.all(orders.map(async (order) => {
        ...
        order.trackingEvents.push(createStatusTrackingEvent(ORDER_STATUSES.FAILED));
        await order.save();
    }));
    ...
    await notifyUsers([{
        ...
        emailTitle: "Your Wilwa payment did not go through",
...
export const expireStalePendingGatewayOrders = async ({ olderThanMinutes = 30 } = {}) => {
    await connectDB();

    const cutoff = new Date(Date.now() - Math.max(5, Number(olderThanMinutes) || 30) * 60 * 1000);
    ...
    staleOrders.forEach((order) => {
        const key = order.paymentReference || String(order._id);
        if (!groups.has(key)) {
            groups.set(key, []);
        }
    ...
    for (const orders of groups.values()) {
        await failOrders(orders, orders.find((order) => order.paymentTransactionId)?.paymentTransactionId || "expired");
        expiredOrderCount += orders.length;
    }

    return {
        checkedOrderCount: staleOrders.length,
        expiredOrderCount,
        expiredCheckoutCount: groups.size,
    };
};
...
//
// A transaction id is preferred, but gateways that only mint one once the
// shopper has actually paid (Flutterwave) can be asked about our own reference
// instead, so a late webhook never strands an order in Pending.
export const settleTransaction = async ({ gateway, transactionId, fallbackReference = "" }) => {
    const verified = transactionId
        ? await gateway.verify(transactionId)
        : gateway.verifyByReference && fallbackReference
            ? await gateway.verifyByReference(fallbackReference)
            : null;
