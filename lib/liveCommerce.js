...
const emptyFlashDealSnapshot = {
  flashDealActive: false,
  flashDealEndsAt: 0,
  flashDealEndsInMs: 0,
  flashDealCountdownLabel: "",
  hasFlashDealDeadline: false,
  flashDealNotStarted: false,
};

// A flash deal only counts as active when it's flagged, has a genuine
// discount (price > offerPrice), has started (if a start date is set), and
// hasn't passed its end date. This mirrors the server-side auto-expiry cron
// (config/inngest.js `expireFlashDeals`) so the UI is always correct even in
...
const getFlashDealSnapshot = (product) => {
  ...
  if (!flaggedAsFlashDeal) {
    return emptyFlashDealSnapshot;
  }

  const price = getPriceValue(product?.price);
  const offerPrice = getPriceValue(product?.offerPrice);
  const hasValidDiscount = price > offerPrice && offerPrice > 0;
  if (!hasValidDiscount) {
    return emptyFlashDealSnapshot;
  }

  const now = Date.now();
  const startsAt = product?.flashDealStartDate ? new Date(product.flashDealStartDate).getTime() : 0;
  const hasNotStarted = Number.isFinite(startsAt) && startsAt > now;
  if (hasNotStarted) {
    return { ...emptyFlashDealSnapshot, flashDealNotStarted: true };
  }
  ...
  const flashDealEndsInMs = hasFlashDealDeadline ? Math.max(0, endsAt - now) : 0;
...
export const getFlashDealTimeParts = (milliseconds) => {
...
const TRENDING_MIN_LIKES = 8;
const TRENDING_MIN_REVIEWS = 5;
const TRENDING_MIN_RATING = 4;

export const SYSTEM_TAG_DEFINITIONS = [
  { slug: "new-arrival", name: "New Arrival", color: "green" },
  { slug: "trending", name: "Trending", color: "purple" },
  { slug: "flash-deal", name: "Flash Deal", color: "red" },
];

export const getSystemTags = (product) => {
  const activity = getProductActivitySnapshot(product);
  const tags = [];

  if (activity.freshnessTier === "today" || activity.freshnessTier === "week") {
    tags.push({ ...SYSTEM_TAG_DEFINITIONS[0], system: true });
  }

  const isTrending = activity.likesCount >= TRENDING_MIN_LIKES
    || (activity.hasRating && activity.reviewCount >= TRENDING_MIN_REVIEWS && activity.displayRating >= TRENDING_MIN_RATING);
  if (isTrending) {
    tags.push({ ...SYSTEM_TAG_DEFINITIONS[1], system: true });
  }

  if (activity.flashDealActive) {
    tags.push({ ...SYSTEM_TAG_DEFINITIONS[2], system: true });
  }

  return tags;
};

export const resolveProductTagSlugs = (product) => {
  const manualSlugs = Array.isArray(product?.tags) ? product.tags : [];
  const systemSlugs = getSystemTags(product).map((tag) => tag.slug);
  return [...new Set([...manualSlugs, ...systemSlugs])];
};
