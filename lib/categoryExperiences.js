...
// No products[0] fallback here on purpose — falling back to an arbitrary
// unrelated product (e.g. the first item in the whole catalog) is what used
// to make completely unrelated tiles all show the same photo. If nothing in
// this category matches, the tile just shows its icon instead (see
// CategoryTile in components/CategoryBrowserPage.jsx and the equivalent
// icon fallback wherever these tiles render).
const findRepresentativeProduct = (products, categories, keywords, fallbackCategory) => (
  products.find((product) => {
    const haystack = `${normalizeText(product?.name)} ${normalizeText(product?.description)} ${normalizeText(product?.category)}`;
    return categories.some((category) => normalizeText(product?.category).includes(normalizeText(category)))
      || keywords.some((keyword) => haystack.includes(normalizeText(keyword)));
  }) || products.find((product) => normalizeText(product?.category).includes(normalizeText(fallbackCategory))) || null
);
...
export const getCategoryExperience = (products, categoryValue) => {
  ...
  return {
    ...
    featuredProduct: tiles.find((tile) => tile.product)?.product
      || products.find((product) => normalizeText(product?.category).includes(normalizeText(meta.value)))
      || null,
  };
};
