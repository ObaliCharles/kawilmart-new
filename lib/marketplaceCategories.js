...
const categoryMetaByAlias = new Map(
    flatCategories.flatMap((category) =>
...
export const getCategoryMeta = (value) =>
    categoryMetaByValue.get(value) || categoryMetaByAlias.get(value) || {
        value,
...
// Merges the static home rail with the admin-created top-level Category
// records. An admin record whose name equals a static value enriches that
// entry (icon/image) instead of appending a second one — concatenating the two
// lists produced duplicate React keys and a doubled category rail whenever an
// admin recreated a name that already shipped statically.
export const buildTopCategoryRail = (customTopCategories = []) => {
    ...
    const staticRail = homeCategoryValues.map((value) => {
        ...
        return {
            value,
            label: meta.label,
            icon: dbMatch?.icon || meta.icon,
            imageUrl: dbMatch?.imageUrl || "",
        };
    });

    const customRail = [];
    (customTopCategories || []).forEach((category) => {
        // Guards both collisions with the static list and duplicate admin
        // records sharing a name.
        if (!category?.name || usedNames.has(category.name)) return;
...
export const categoryMatchesSelection = (productCategory, selectedCategory) => {
    ...
    const selectedAliasMeta = categoryMetaByAlias.get(selectedCategory);
    if (selectedAliasMeta?.value === productCategory || selectedAliasMeta?.aliases.includes(productCategory)) {
        return true;
    }

    const productAliasMeta = categoryMetaByAlias.get(productCategory);
    if (productAliasMeta?.value === selectedCategory || productAliasMeta?.aliases.includes(selectedCategory)) {
        return true;
    }
...
// Homepage "Top Categories" rail (mobile): the default quick-pick tiles and
// the sentinel parentValue under which admins can attach uploaded PNG icons
// to them (or add new tiles) from Admin -> Categories.
export const TOP_RAIL_PARENT = "__top";

export const homeTopRailDefaults = [
    { label: "T-Shirt", category: "Fashion" },
    { label: "Shirt", category: "Fashion" },
    { label: "Shoes", category: "Fashion" },
    { label: "Watches", category: "Watches & Wearables" },
    { label: "Bag", category: "Accessories" },
    { label: "Jeans", category: "Fashion" },
