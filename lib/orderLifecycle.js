...
// How the buyer chose to pay. COD is settled in person; mobile-money methods
// are recorded now and wired to a gateway later (see lib/payments.js).
...
export const normalizePaymentMethod = (value) => (
    Object.values(PAYMENT_METHODS).includes(value) ? value : PAYMENT_METHODS.COD
);

// Returns/refunds: buyer can request within the return window after delivery;
// the seller (or admin) approves or rejects; refunds are marked once settled.
...
export const canCustomerRequestReturn = (order = {}) => {
    const status = normalizeOrderStatus(order?.status);
    if (status !== ORDER_STATUSES.DELIVERED && status !== ORDER_STATUSES.COMPLETED) {
        return false;
    }

    if ((order?.returnRequest?.status || RETURN_STATUSES.NONE) !== RETURN_STATUSES.NONE) {
        return false;
    }

    const deliveredAt = order?.deliveredAt || order?.customerConfirmedAt || order?.date;
    if (!deliveredAt) {
        return false;
    }

    const ageMs = Date.now() - new Date(deliveredAt).getTime();
    return Number.isFinite(ageMs) && ageMs >= 0 && ageMs <= RETURN_WINDOW_DAYS * 24 * 60 * 60 * 1000;
};

export const canSellerResolveReturn = (order = {}) => (
    (order?.returnRequest?.status || RETURN_STATUSES.NONE) === RETURN_STATUSES.REQUESTED
);
